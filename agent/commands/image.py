...
class ImageCommands:
    ...
    def extract_file(self, params: dict) -> dict:
        ...
        try:
            ...
            logger.info("Pulling image %s for file extraction (platform=%s)", image, platform)
        ...
        cmd = ["sh", "-c", f"cat {shlex.quote(path)} 2>/dev/null || echo {_NOT_FOUND}"]
        ...
        try:
            ...
            logger.warning("extract_file failed for %s:%s — %s", image, path, exc)
            return {"content": None}
